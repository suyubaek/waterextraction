...
class ASPPConvWithSepConv(nn.Sequential):
    """
    使用深度可分离卷积的ASPP卷积模块
    """
    def __init__(self, in_channels, out_channels, dilation):
        ...
        super(ASPPConvWithSepConv, self).__init__(*modules)
...
class ASPP(nn.Module):
    ...
    def __init__(self, in_channels, atrous_rates, out_channels=256):
        ...
        for rate in rates:
            modules.append(ASPPConvWithSepConv(in_channels, out_channels, rate))
...
class DecoderWithSepConv(nn.Module):
    """
    使用深度可分离卷积的DeepLabV3+解码器
    将编码器的高层特征与低层特征融合
    """
    def __init__(self, low_level_channels, encoder_channels):
        super(DecoderWithSepConv, self).__init__()
        ...
        # 使用深度可分离卷积的输出卷积
        self.output_conv = nn.Sequential(
            DepthwiseSeparableConv(encoder_channels + 48, 256, kernel_size=3, padding=1),
            nn.Dropout(0.5),
...
class DeepLabV3Plus(nn.Module):
    ...
    def __init__(self, in_channels=3, num_classes=1, output_stride=16):
        ...
        # 解码器
        self.decoder = DecoderWithSepConv(64, 256)
